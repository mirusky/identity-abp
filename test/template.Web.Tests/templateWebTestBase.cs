...
{
    public abstract class templateWebTestBase : AbpAspNetCoreIntegratedTestBase<Startup>
    {
        protected static readonly Lazy<string> ContentRootFolder;

        static templateWebTestBase()
        {
        ...
        protected override IWebHostBuilder CreateWebHostBuilder()
        {
            return base
                ...
                .UseSetting(WebHostDefaults.ApplicationKey, typeof(templateWebMvcModule).Assembly.FullName);
        }
        ...
        {
            using (var context = IocManager.Resolve<templateDbContext>())
            {
                action(context);
                context.SaveChanges();
            }
        ...
        protected async Task UsingDbContextAsync(Func<templateDbContext, Task> action)
        {
            using (var context = IocManager.Resolve<templateDbContext>())
            {
        ...
        protected async Task<T> UsingDbContextAsync<T>(Func<templateDbContext, Task<T>> func)
        {
            T result;
